...
class Agm_AdminFormRenderer {
	...
	public function create_map_api_key_box() {
		...
		echo ' ';
		printf(
			__('(Du kannst auch ein wenig mehr in den <a href="%s" target="_blank">offiziellen Google-Dokumentationsseiten</a> recherchieren)', AGM_LANG),
	...
	public function create_map_zoom_box() {
		...
		<div id="agm-zoom-basic-container" <?php echo '' . $basic_visibility; ?>>
			...
			<?php _e(
				'<div>Bitte beachten Sie, dass diese Titel nur Annäherungen sind, aber '.
				'allgemein zur Beschreibung passen.</div>', AGM_LANG
			); ?>
		</div>
		<?php
	...
	public function create_map_units_box() {
		...
		echo '</select>';
		_e( '<div>Diese Einheiten werden verwendet, um Entfernungen für Wegbeschreibungen auszudrücken</div>', AGM_LANG );
	}
	...
	public function create_custom_css_box() {
		...
		<?php
		_e(
			'<p>Du kannst dieses Feld verwenden, um einige schnelle Stiländerungen hinzuzufügen, ' .
			'um das Erscheinungsbild der Karten besser an Deine Themes anzupassen.</p>', AGM_LANG
		);
		_e(
			'<p>Möglicherweise möchtest Du für einige dieser Selektoren Stile festlegen: ' .
	...
	public function create_alt_shortcode_box() {
		...
		<p>
			...
			<?php _e(
				...
				'"Rename map shortcode". Falls erforderlich wird dieses Tool ' .
				'automatisch alle Deine Seiten und Beiträge aktualisieren, um ' .
	...
	public function create_custom_fields_map_box() {
		...
		echo '<br />';
		_e( 'Name des Längenfelds:', AGM_LANG );
		...
		echo '&nbsp;';
		echo '<label for="agm-custom_fields-discard_old">' . __( 'Alte Zuordnung verwerfen, wenn sich der Wert meiner benutzerdefinierten Felder ändert', AGM_LANG ) . '</label>';
	}
	...
	public function create_custom_fields_options_box() {
		...
		printf(
			__( 'Wenn die vorherige Option auf „Ja“ gesetzt ist, wird die neue Karte %s im Beitragstext angezeigt.', AGM_LANG ),
			$select
		);
	}
	...
	public function create_overview_box() {
		...
		<p>
			<?php printf(
				__(
					'PS Maps fügt ein &quot;Add Map&quot; Icon ' .
					'zu deinem visuellen Editor hinzu. Sobald du deine ' .
					'neue Karte erstellt hast, wird sie in den Beitrag / Seitenbereich als ' .
					'Shortcode eingefügt, der so aussieht: <code>[%s id="1"]</code>.',
					AGM_LANG
				),
				$shortcode_tag
			); ?>
		...
		<p>
			<?php _e(
				'Es fügt auch ein Widget hinzu, mit dem du Karten zu deiner ' .
				'Seitenleiste hinzufügen kannst (siehe Design &gt; Widgets).', AGM_LANG
			); ?>
		</p>
		<?php
		...
		if ( ! is_multisite() ) {
			...
			<p>
				<?php printf(
					__(
						'Für detailliertere Anweisungen zur Verwendung ' .
						'siehe <a target="_blank" href="%s">PS Maps ' .
						'Installations- und Gebrauchsanweisungen</a>.', AGM_LANG
					),
					'https://cp-psource.github.io/ps-maps//#usage'
				); ?>
			</p>
			<?php
		}
	...
	public function create_plugins_box() {
		...
		foreach ( $sections as $section ) : ?>
			...
			<th width="70%"><?php _e( 'Add-on-Beschreibung', AGM_LANG ); ?></th>
			</tr>
			<?php echo '</' . $section . '>'; ?>
		<?php endforeach;
