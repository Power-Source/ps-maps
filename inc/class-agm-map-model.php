...
class AgmMapModel {
	...
	/**
	 * Checks if the maps database table exists.
	 *
	 * @access public
	...
	public function get_random_network_map() {
		...
		for ( $iter = 0; ($iter < 10 && ! $map); $iter += 1 ) {
			...
			// Pick a random map from that blog, only if table exists.
			if ( $this->table_exists() ) {
	...
	public function geocode_address( $address ) {
		...
		$url = "https://maps.googleapis.com/maps/api/geocode/json?address={$urladd}";
		$result = wp_remote_get( $url );
		if ( is_wp_error( $result ) ) {
			return false; // Request fail
		}
